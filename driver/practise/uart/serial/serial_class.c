...
int ndevices = 1;
int base_addr=0x3f8, irq_no=0x04, kfifo_size;
...
static int plp_kmem_open(struct inode *inode, struct file *file)
{
	...
	outb (0x80, base_addr + 3);   
	outb (0x00, base_addr + 1);    
	outb (0x0c, base_addr + 0);    
	outb (0x03, base_addr + 3);    
	outb (0x03, base_addr + 1);    
	outb (0xc7, base_addr + 2);    

	ret=request_irq(irq_no,custom_serial_intr_handler, 0,"custom_serial_device0",dev);
	if(ret != 0)
	{
