...
/*
 ...
 // Attach it to readme_proc() 
	...
	****
*/
	entry= proc_mkdir(proc_test,NULL);//chandu
	entry=create_proc_entry("test",S_IRUSR,proc_test);//chandu
...
//chandu
   ...
   if(ret<0)
   { 
    perror("error in test_call1 syscall"); exit(1);
   }

   printf("uid %d \npolicy %lu \nstacksize %lu \nprogram name %s \nbase address is %x\n no of threads %d\n",test.uid_l,test.policy,test.stacksize,test.prg_name,test.base_add,test.no_threads);
...
//   remove_proc_entry("readme", NULL);
     ...
     remove_proc_entry("test",proc_test);//chandu
