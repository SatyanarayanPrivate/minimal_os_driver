...
int main()
{
   ...
   if(ret>0) { printf("error in thread4 creation for producer\n"); exit(4); }   
   ...
   if(ret>0) { printf("error=%d in thread1 creation for consumer\n",ret); exit(1); }

   ret = pthread_create(&thid2,NULL,consumer_thread,NULL);
   if(ret>0) { printf("error in thread2 creation for consumer\n"); exit(2); } 

   ret = pthread_create(&thid3,NULL,producer_thread,NULL);
   if(ret>0) { printf("error in thread3 creation for producer\n"); exit(3); } 
