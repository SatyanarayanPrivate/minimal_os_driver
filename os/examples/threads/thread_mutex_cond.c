...
//here we use a mutex to solve the typical critical section problem
//we may use one or more mutexes as per need - here, just one will 
//do !!!
...
//these are the condition variable objects !!!
//we may use one or more of these condition variables, as needed !!!
//condition variable is peculiar - it does not maintain a counter -
//it does not maintain a lock variable - it just maintains 
//a wq - its operations work along with a mutex !!!
...
//for a processes, the typical message queue object is resident
//in system space - for obvious reasons 

//for a set of threads of the same process, a developer defined 
//message queue object is resident in the global data area of 
//the processes - visible to all threads - meaning, sharable
//among all threads !!!it is shareable and shared !!!
...
//each message is defined as required by the developer 
	...
	while(1)
	{
		struct job* next_job;
        	...
        	pthread_mutex_lock(&m1);
                ...
                //woken up this thread via pthread_cond_signal - 
                //this thread will wake up and resume  
                ...
                //resume the execution  by returning from 
                //pthread_cond_wait() - meaning, thread resumes
                //execution after pthread_cond_wait() and continues !!!
...
//		printf("Enter the string to be Queued \n");
	...
	//	gets(new_job->str1);
		...
		pthread_mutex_unlock(&m1); //after wake up of a condition 
                ...
                //sem_post(&sem1);
               //sched_yield() may be used with processes or thread,
...
int main()
{
	...
	pthread_mutexattr_init(&ma1);//initialize to default attributes !!
        ...
        //using appropriate APIs, we can selectively modify 
        //one or more attributes of a semaphore 
        //
        //below API sets the type attribute of the mutex
        //to NORMAL
	//pthread_mutexattr_settype(&ma1,PTHREAD_MUTEX_NORMAL);

        //this sets the type attribute to ERRORCHECKING 
	...
	//pthread_mutex_init(&m1,NULL);
        ...
        //normal mutex type behaviour 
        //
        //assuming that our mutex type is NORMAL, most of the 
        //standard operations on the mutex is as above 
        //in addition, if a NORMAL mutex is already locked 
        //by a thread and the same thread attempts to lock 
        //the same mutex without unlocking, the thread will
        //be dead-locked !!!

        //if a mutex is locked by a thread of a process and 
        //another thread of the same process attempts to unlock 
        //the mutex, the consequence is not defined !!!!

        //if a mutex is unlocked by a thread and again unlocked
        //by the same thread or another, the consequence is not 
        //defined !!! 

        //error checking  mutex type behaviour 
        //
        //assuming that our mutex type is ERRORCHECKING, most of the 
        //standard operations on the mutex is as above 
        //in addition, if a EC mutex is already locked 
        //by a thread and the same thread attempts to lock 
        //the same mutex without unlocking, the thread will
        //not be blocked and pthread_mutex_lock() will 
        //return appropriate error code  !!!

        //if a mutex is locked by a thread of a process and 
        //another thread of the same process attempts to unlock 
        //the mutex, pthread_mutex_unlock() will not unlock,
        //but return an appropriate error code  !!!!

        //if a mutex is unlocked by a thread and again unlocked
        //by the same thread or another, pthread_mutex_unlock()
        //will return appropriate error code

        //in the case of EC mutex, it is the responsibility of the
        //developer to check for errors and fix them during 
        //development - such interesting facilities are typically
        //not available in the case of semaphores !!!
