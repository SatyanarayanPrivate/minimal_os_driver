...
//although the below approach of using mutexes is ok,
//use the intialization techniques of thread_mutex_cond.c 
//- the techniques used there are better !!
...
//like mutex objects, these light weight semaphore objects are 
//in user-space 
...
//refer to man of sem_init()
//
//second parameter is a flag - based on this flag, the semaphore
//may be used between threads of the same process or threads of 
//different processes - we will see more of this in man page
//and our discussion !!
//
//param3 is used to intialize the semaphores value as per 
...
//2 semaphores are used as counting semaphores in the thread methods 
//below - these counting semaphores work on the same principle 
//that we studied during process semaphores 
//
...
//in this thread, mutex is used to lock a critical section 
//using a shared buffer
//semaphore is used to implement a synchronization semaphore
//it is not a lock - it is not a counting semaphore  
//
void *user_input_thread(void *arg)
{
   ...
   while(1){
     ...
     //following is the typical API used to lock a mutex
     //you must add error checking to this API 
     ...
     //printf("data input is %s\n", user_input_buffer);
     //this sem_post() is an increment operation controlling
     //the execution of the next thread that is blocked 
     //on sem_wait(), with initial value set to 0 !!!
     sem_post(&seminput);//V()
...
void *producer_thread(void *arg)
{
   ...
   while(1){
     ...
     //using the sem_wait() decrement operation and the initial
     //value set to 0, this thread's execution is controller by 
     //the previous thread - this is a typical example for
     //synchronization !!!
...
//a typical thread is normally terminated using pthread_exit()
//pthread_exit() can be passed a pointer as return value from 
//the thread - do not confuse this with exit code - this pointer
//can be collected by pthread_join(), if this thread is joinable 
//
//however, if a thread encounters serious errors/bugs, it is the
//responsiblity of the developer to invoke exit(n), instead 
//when to use pthread_exit() and when to exit(n) is a matter
//of skill !!!
...
int main()
{
   ...
   //created
   //param1 is pointer to a user-space thread id object of the
   //thread library - do not confuse this with system space
   //thread id !!  

   //param2 is used to set the attributes of the newly 
   //created thread by this API - we will see later
   //if param2 is NULL, default attributes are used by the 
   //library for a newly created thread !!
   ...
   //thread's method whose address is passed as param3 - 
   //developer is free to pass NULL, if
   //not needed
   ...
   //ideally, we must use sterror_r() to print the error messages !!
 ...
 ret = pthread_create(&thid3,NULL,producer_thread,NULL);
   ...
   //by any  thread of this process - first param is lib thread id - 
   //pthread_join() will block the current thread until 
   //target thread whose thread
